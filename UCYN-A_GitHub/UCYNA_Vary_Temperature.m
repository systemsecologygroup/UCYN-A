...
T_range = -5:5:35; % Range of light temperature
...
J_L_max_H = C_H*(p_a+log10(vol_H))/(p_b - p_c*log10(vol_H)+log10(vol_H)^2); % fmol C/cell/d (Cheung et al 2022)
...
Nfix = zeros(1,length(T_range));
div_rate = zeros(1,length(T_range));

S_J_C = zeros(1,length(T_range)); 
S_J_N = zeros(1,length(T_range));
S_J_SC = zeros(1,length(T_range));
S_R = zeros(1,length(T_range));
S_g = zeros(1,length(T_range));

S_R_basal = zeros(1,length(T_range)); 
S_R_Fe = zeros(1,length(T_range)); 
S_R_P = zeros(1,length(T_range)); 
S_R_N2 = zeros(1,length(T_range)); 
S_R_O2 = zeros(1,length(T_range)); 
S_R_SC = zeros(1,length(T_range)); 

S_O2 = zeros(1,length(T_range)); 

H_J_N = zeros(1,length(T_range));
H_J_L = zeros(1,length(T_range));
H_J_C = zeros(1,length(T_range));
H_R = zeros(1,length(T_range));
H_g = zeros(1,length(T_range));

C_transfer = zeros(1,length(T_range));
N_transfer = zeros(1,length(T_range));

for i1 = 1:length(T_range)
    
    temp_current = T_range(i1)

    %%%%%%%%%%%%% Q10 %%%%%%%%%%%

    Q10_J_L = a3 - a4*(1./(1+exp(-a1*temp_current))).^a2;
%     Q10_R = b3 + b4*(1./(1+exp(-b1*temp))).^b2;
    ...
    S_J_C_all = zeros(length(theta_range),length(phi_range));
    S_J_N_all = zeros(length(theta_range),length(phi_range));
    S_J_SC_all = zeros(length(theta_range),length(phi_range));
    S_R_all = zeros(length(theta_range),length(phi_range));
    S_g_all = zeros(length(theta_range),length(phi_range));

    S_R_basal_all = zeros(length(theta_range),length(phi_range));
    S_R_Fe_all = zeros(length(theta_range),length(phi_range));
    S_R_P_all = zeros(length(theta_range),length(phi_range));
    S_R_N2_all = zeros(length(theta_range),length(phi_range));
    S_R_O2_all = zeros(length(theta_range),length(phi_range));
    S_R_SC_all = zeros(length(theta_range),length(phi_range));

    S_O2_all = zeros(length(theta_range),length(phi_range));

    H_J_N_all = zeros(length(theta_range),length(phi_range));
    H_J_L_all = zeros(length(theta_range),length(phi_range));
    H_J_C_all = zeros(length(theta_range),length(phi_range));
    H_R_all = zeros(length(theta_range),length(phi_range));
    H_g_all = zeros(length(theta_range),length(phi_range));

    C_transfer_all = zeros(length(theta_range),length(phi_range));
    N_transfer_all = zeros(length(theta_range),length(phi_range));
    ...
    for i = 1:length(theta_range)
        ...
        for j = 1:length(phi_range)
            ...
            div_H_temp(n_CN*J_HN_temp > J_HG_temp) = 0; % to restrict excess N2 fixation to just increase respiration to burn O2
            ...
            S_J_C_all(i,j) = theta*J_HC_bar_temp;
            S_J_N_all(i,j) = J_SN_temp;
            S_J_SC_all(i,j) = J_SC_temp;
            S_R_all(i,j) = R1_temp;
            S_g_all(i,j) = J_SG_temp;

            S_R_basal_all(i,j) = R_0_Q10*C_S;
            S_R_Fe_all(i,j) = R_Fe_Q10*J_Fe_S;
            S_R_P_all(i,j) = R_P_Q10*J_P_S;
            S_R_N2_all(i,j) = R_N2_Q10*J_SN_temp;
            S_R_O2_all(i,j) = R_O2_temp;
            S_R_SC_all(i,j) = Resp_S_temp;

            S_O2_all(i,j) = D_S*O2_H; % mumol O2/cell/d

            H_J_N_all(i,j) = rho_N_temp;
            H_J_L_all(i,j) = J_L;
            H_J_C_all(i,j) = J_HC_temp;
            H_R_all(i,j) = Resp_H;
            H_g_all(i,j) = J_HG_temp;

            C_transfer_all(i,j) = theta*J_HC_bar_temp/J_L*100;
            N_transfer_all(i,j) = rho_N_temp/J_SN_temp*100;
    ...
    div_rate(i1) = div_rate_H_temp(ind_row,ind_col);

    Nfix(i1) = phi_range(ind_col)*J_Nfix_max;

    S_J_C(i1) = S_J_C_all(ind_row,ind_col);
    S_J_N(i1) = S_J_N_all(ind_row,ind_col);
    S_J_SC(i1) = S_J_SC_all(ind_row,ind_col);
    S_R(i1) = S_R_all(ind_row,ind_col);
    S_g(i1) = S_g_all(ind_row,ind_col);

    S_R_basal(i1) = S_R_basal_all(ind_row,ind_col);
    S_R_Fe(i1) = S_R_Fe_all(ind_row,ind_col);
    S_R_P(i1) = S_R_P_all(ind_row,ind_col);
    S_R_N2(i1) = S_R_N2_all(ind_row,ind_col);
    S_R_O2(i1) = S_R_O2_all(ind_row,ind_col);
    S_R_SC(i1) = S_R_SC_all(ind_row,ind_col);

    S_O2(i1) = S_O2_all(ind_row,ind_col);

    H_J_N(i1) = H_J_N_all(ind_row,ind_col);
    H_J_L(i1) = H_J_L_all(ind_row,ind_col);
    H_J_C(i1) = H_J_C_all(ind_row,ind_col);
    H_R(i1) = H_R_all(ind_row,ind_col);
    H_g(i1) = H_g_all(ind_row,ind_col);

    C_transfer(i1) = C_transfer_all(ind_row,ind_col);
    N_transfer(i1) = N_transfer_all(ind_row,ind_col);
...
plot(T_range,Nfix,'Color','b','LineWidth',2);
hold on;
