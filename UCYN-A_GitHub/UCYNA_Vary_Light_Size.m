...
frac_variation = 0.15; % 15% variation of diameters

div_diam = 10; % Number of division within the range of variation of diameters
...
diam_H_range = linspace(diam_H - frac_variation*diam_H, diam_H + frac_variation*diam_H, div_diam); % mum
diam_S_range = linspace(diam_S - frac_variation*diam_S, diam_S + frac_variation*diam_S, div_diam); % mum

Nfix_1 = zeros(length(L_range),length(diam_H_range),length(diam_S_range));

for j1 = 1:length(diam_H_range)
    j1
    tic
    diam_H = diam_H_range(j1);

    rad_H = diam_H/2; % mum
    vol_H = 4/3*pi*(rad_H)^3; % 1.56; % mum^3

    for j2 = 1:length(diam_S_range)

        diam_S = diam_S_range(j1);

        rad_S = diam_S/2; % mum
        vol_S = 4/3*pi*(rad_S)^3; % 1.56; % mum^3
        ...
        for i1 = 1:length(L_range)
            ...
            phi_val = phi_range(ind_col);
                        
            Nfix_1(i1,j1,j2) = phi_val*J_Nfix_max;
